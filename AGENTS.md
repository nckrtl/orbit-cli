...
- Run Artisan commands directly via the command line (e.g., `php orbit route:list`). Use `php orbit list` to discover available commands and `php orbit [command] --help` to check parameters.
- Inspect routes with `php orbit route:list`. Filter with: `--method=GET`, `--name=users`, `--path=api`, `--except-vendor`, `--only-vendor`.
- Read configuration values using dot notation: `php orbit config:show app.name`, `php orbit config:show database.default`. Or read config files directly from the `config/` directory.

## Tinker

- Execute PHP in app context for debugging and testing code. Do not create models without user approval, prefer tests with factories instead. Prefer existing Artisan commands over custom tinker code.
- Always use single quotes to prevent shell expansion: `php orbit tinker --execute 'Your::code();'`
  - Double quotes for PHP strings inside: `php orbit tinker --execute 'User::where("active", true)->count();'`
...
- Use `php orbit make:` commands to create new files (i.e. migrations, controllers, models, etc.). You can list available Artisan commands using `php orbit list` and check their parameters with `php orbit [command] --help`.
